...
void Playernumber(int turn) { cout << "Ход игрока " << turn << endl; }
...
void switchPlayerTurn(int *turn) { *turn = 3 - *turn; }
...
void menu() {
  ...
  cout
      << "Из кучки, первоначально содержащей 100 спичек, два игрока поочередно "
  ...
  while (p < 1) {
    ...
    if (a == 2) {
      ...
      cout << "Выбирите другую цифру" << endl;
    }
  }
}
